...
const getResult = (n) => {
  const options = [
    ...
    {
      amount: n * 100,
      msg: `৳100 per person. The budget option. Still counts.`,
    },
...
export default function SalamiCalculator() {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs mb-5 leading-relaxed text-center text-theme-subtle">
      ...
      <div className="flex gap-2 mb-5">
        <input
          ...
          placeholder="e.g. 12"
          className="flex-1 min-w-0 px-4 py-2.5 rounded-xl border text-sm font-semibold placeholder:text-theme-subtle focus:outline-none focus:ring-2 focus:ring-emerald-500/20 text-theme-secondary border-theme bg-surface w-full"
        />
        <button
          onClick={calculate}
          disabled={!count || parseInt(count) < 1}
          className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white text-xs font-bold transition-all duration-150 active:scale-95 shrink-0"
        >
          Calculate
        </button>
      </div>
      ...
      <div className="min-h-[72px] flex items-center w-full mt-auto">
        <AnimatePresence mode="wait">
          {result ? (
            ...
            >
              <p className="text-2xl font-extrabold mb-1 text-brand-theme text-center">
                ৳{result.amount.toLocaleString()}
              </p>
              <p className="text-xs italic text-theme-muted text-center">
                {result.msg}
              </p>
            </motion.div>
          ) : (
